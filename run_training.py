...
train_dataset = CustomDataset(train_data, n_atomtype=10, Zonehot_scale=1, Zcharge_scale=1)
train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=64, shuffle=True, drop_last=True)
val_dataset = CustomDataset(val_data, n_atomtype=10, Zonehot_scale=1, Zcharge_scale=1)
...
# train_data = {'R': np.tile(np.array([[0,0,0],[0,0,1]]), [10000, 1, 1]),
#               'Z': np.tile(np.array([[1],[1]]), [10000, 1, 1])}
# val_data = {'R': np.tile(np.array([[0,0,0],[0,0,1]]), [10000, 1, 1]),
#             'Z': np.tile(np.array([[1],[1]]), [10000, 1, 1])}

# model = Diffusion(n_layer=3, n_feat=32, atomtype=[1])
# device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# train_dataset = CustomDataset(train_data, atomtype=[1])
# train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=64, shuffle=True, drop_last=True)
# val_dataset = CustomDataset(val_data, atomtype=[1])
# val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=64, shuffle=True, drop_last=True)
